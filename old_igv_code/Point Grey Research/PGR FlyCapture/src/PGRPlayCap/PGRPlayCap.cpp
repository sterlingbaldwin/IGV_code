...
HRESULT CaptureVideo()
{
	...
	hr = SetFlyStreamCamera(pSrcFilter);
	if (FAILED(hr))
    {
        // Don't display a message because SetFlyStreamCamera will handle it
        return hr;
    }
...
//
//Example Function to demonstrate how to interact with the FlyStreamFilter
//
HRESULT SetFlyStreamCamera(IBaseFilter * pSrcFilter)
{
	HRESULT hr;
	IFlyCaptureProperties *pProps = NULL;

	unsigned int Mode = 0, uiMaxWidth =0 , uiMaxHight = 0,uiPixFormats = 0;
	bool bAvailable = false;

	//
	// Get a pointer to the FlyStreamProperties interface
	//
	hr = pSrcFilter->QueryInterface(IID_IFlyCaptureProperties,(void**)&pProps);
	if (FAILED(hr))
    {
		Msg(TEXT("Couldn't Query the Capture Interface!  hr=0x%x"), hr);
		return E_FAIL;
	}	

	//
	// Query the Custom image Setings
	//
	hr = pProps->QueryCustomImage(0,&bAvailable,&uiMaxWidth,&uiMaxHight,&uiPixFormats);
	if (FAILED(hr))
    {
		Msg(TEXT("Couldn't Query the caustom image!  hr=0x%x"), hr);
		SAFE_RELEASE(pProps);
		return E_FAIL;
	}	

	//
	// Verify that all the custom image settings we want to use are supported
	//
	if( uiPixFormats & FLYCAPTURE_MONO16 && bAvailable 
		&& DEFAULT_VIDEO_WIDTH <= uiMaxWidth && DEFAULT_VIDEO_HEIGHT <= uiMaxHight)
	{
		//
		// Put the Camera into Custom Image mode
		//
		hr = pProps->SetCustomImageMode(true);
		if (FAILED(hr))
		{
			Msg(TEXT("Couldn't Set Custom Image Mode!  hr=0x%x"), hr);
			SAFE_RELEASE(pProps);
			return E_FAIL;
		}
		...
		//
		hr = pProps->SetCustomImage(Mode,0,0,DEFAULT_VIDEO_WIDTH,DEFAULT_VIDEO_HEIGHT,FLYCAPTURE_MONO16);
		if (FAILED(hr))
		{
			Msg(TEXT("Couldn't Set the Custom Image!  hr=0x%x"), hr);
			SAFE_RELEASE(pProps);
			return E_FAIL;
		}
	...
	{
		Msg(TEXT("No such format available!  hr=0x%x"), hr);
		SAFE_RELEASE(pProps);
		return E_FAIL;
	}
...
HRESULT FindCaptureDevice(IBaseFilter ** ppSrcFilter)
{
	...
	while (pClassEnum->Next(1, &pMoniker, NULL) == S_OK)
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			LPCTSTR szName = OLE2T(varName.bstrVal);

			if( lstrcmp(szName , L"Point Grey Camera") ==0 )
			{
	...
	if( pSrc == NULL )
	{
		Msg(TEXT("Couldn't Find a FlyStream Object!  hr=0x%x"), hr);
		hr = E_FAIL;
	}
	else
	{
