...
//=============================================================================
//=============================================================================
// $Id: main.cpp,v 1.1 2007/10/30 18:07:34 demos Exp $
//=============================================================================

//=============================================================================
//
// MultipleCameraPlusEx
//
...
doGrabLoop()
{
   ...
   unsigned int	aruiDeltaTimestamp[ _MAX_CAMERAS ];
   unsigned int	aruiPrevTimestamp[ _MAX_CAMERAS ];
   
   unsigned int	aruiPrevFrameNum[ _MAX_CAMERAS ];
   unsigned int	aruiDeltaFrames[ _MAX_CAMERAS ];
   unsigned int	uiMissedFrames    = 0;
   unsigned int	uiDelayedFrames   = 0;
   
   bool		  bFrameCountPresent[ _MAX_CAMERAS ];
   unsigned long  ulFrameInfoValue;
   unsigned int	  uiFrameCountStartByte = 0;
   ...
   for( unsigned int uiImage = 0; uiImage < _IMAGES; uiImage++ )
   {
      ...
      //
      // Since sequence numbers are generated at the driver level, we should
      // also keep track of frame numbers which are populated at the camera.
      // If the frame numbers are not sequential, that means there was a dropped
      // frame between the camera and the driver
      //
      for( uiCamera = 0; uiCamera < g_uiCameras; uiCamera++ )
      {
	 
	 // Check to see if the FrameCount information is available
	 bFrameCountPresent[ uiCamera ] = false;
	 error = flycaptureGetCameraRegister( g_arContext[ uiCamera ], FRAME_INFO, &ulFrameInfoValue  );
	 if ((ulFrameInfoValue & 0x00000040) == 0)
	 {
	    bFrameCountPresent[ uiCamera ] = false;
	 } else {
	    // Since the FrameCount is available, find out how many other components
	    // are being displayed at the beginning of the image.  This will determine
	    // where the FrameCount info starts.
	    bFrameCountPresent[ uiCamera ] = true;
	    uiFrameCountStartByte = 0;
	    for (int i = 0; i < 6; i++)
	    {
	       if ((ulFrameInfoValue & 0x01) == 1) {
		  // Since each value takes 4 bytes, increase the starting byte of the FrameCount.
		  uiFrameCountStartByte += 4;
	       }
	       ulFrameInfoValue = ulFrameInfoValue >> 1;
	    }
	 }
	 
	 // Since we have turned on the frame counter in the FRAME_INFO register
	 // and nothing else, the frame count is located in the first 4 bytes
	 // of the image.  Read all 4 bytes and create a frame count value.
	 // NOTE: if there is more frame info set in FRAME_INFO register,
	 // the frame count will not necessarily be in the first 4 bytes.
	 // See PGR Register Reference for details.
	 unsigned long ulFrameCountValue = 0;
	 
	 if (bFrameCountPresent[ uiCamera ])
	 {
	    for( int iByte = 0; iByte < 4; iByte++ )
	    {
	       ulFrameCountValue <<= 8;
	       ulFrameCountValue += g_arImageplus[ uiCamera ].image.pData[ uiFrameCountStartByte + iByte ];
	    }
	    
	    if (uiImage == 0)
	    {
	       aruiPrevFrameNum[ uiCamera ] = ulFrameCountValue;
	       aruiDeltaFrames[ uiCamera ] = 1;
	    } else {
	       aruiDeltaFrames[ uiCamera ] = ulFrameCountValue - aruiPrevFrameNum[ uiCamera ];
	    }
	 ...
	 if (bFrameCountPresent[ uiCamera ])
	 {
	    ...
	    //
	    // Store the image timestamp in ms
	    //
	    int uiTimestamp = (g_arImageplus[ uiCamera ].image.timeStamp.ulCycleSeconds * 1000) + (g_arImageplus[ uiCamera ].image.timeStamp.ulCycleCount/8);
	    
	    // DeltaTimestamp is in milliseconds so we can compare with the framerate, not in cycles.
	    if (uiImage == 0) {
	       aruiDeltaTimestamp [ uiCamera ] = 0;
	    } else {
	       if ( (uiTimestamp < aruiPrevTimestamp[uiCamera]) )
	       {
		  aruiDeltaTimestamp [ uiCamera ] = (uiTimestamp + 128000) - aruiPrevTimestamp [ uiCamera ];
	       } else {
		  aruiDeltaTimestamp [ uiCamera ] = uiTimestamp - aruiPrevTimestamp [ uiCamera ];
	       }
	    }
	    
	    aruiPrevTimestamp [ uiCamera ] = uiTimestamp;
	    
#ifdef _VERBOSE
	    printf( 
	       "   Camera %02u: sq = %04u, frame = %04x, time = %03u.%04u, deltaT = %u ms\n",
	       uiCamera,
	       g_arImageplus[ uiCamera ].uiSeqNum,
	       ulFrameCountValue,
	       g_arImageplus[ uiCamera ].image.timeStamp.ulCycleSeconds,
	       g_arImageplus[ uiCamera ].image.timeStamp.ulCycleCount,
	       aruiDeltaTimestamp [ uiCamera ]);
#endif
	    
	    if( aruiDeltaFrames[ uiCamera ] != 1 )
	    {
	       // we have missed a frame.
	       uiMissedFrames += aruiDeltaFrames[ uiCamera ] - 1;
	       printf( 
		  "      We have missed a Frame! (Frame %04x, Camera %u)\n", 
		  ulFrameCountValue,
		  uiCamera );
	    }
	    
	    
	    // check to see if we have the ability to read the framerate  
	    float fFrameRate = 0;
	    
	    error = flycaptureGetCameraAbsProperty(
	       g_arContext[ uiCamera ],
	       FLYCAPTURE_FRAME_RATE,
	       &fFrameRate );
	    
	    // if we have the framerate, compare the time between images to the framerate.
	    // If delataT > 1/Framerate, and we have not missed an image, then the image
	    // has been delayed.
	    if ( error == FLYCAPTURE_OK )
	    {
	       float fCompare = (1000 / fFrameRate) + 1;
	       if ( aruiDeltaTimestamp [ uiCamera ] > fCompare ) {
		  
		  // timestamp between images should not be > 1/FrameRate.
		  // If deltaTimestamp is greater, and the frame count has only
		  // incremented by 1, then the frame has been delayed.
		  if( aruiDeltaFrames[ uiCamera ] == 1 )
		  {
		     uiDelayedFrames += 1;
		     printf( 
			"      Delayed frame (Frame %04x, deltaT %u, Expected %u, Camera %u)\n", 
			ulFrameCountValue,
			aruiDeltaTimestamp [ uiCamera ],
			(int)fCompare,
			uiCamera );
   ...
   for( unsigned int uiCamera = 0; uiCamera < g_uiCameras; uiCamera++ ) {
      if (bFrameCountPresent[ uiCamera ])
      {
	 printf( "Missed frames = %u.\n", uiMissedFrames );
	 printf( "Delayed frames = %u.\n", uiDelayedFrames );
	 break;
      }
   }
...
main( int /* argc */, char* /* argv[] */ )
{
   ...
   for( uiCamera = 0; uiCamera < g_uiCameras; uiCamera++ )
   {
      ...
      //
      // Turn on Frame Counter (0x40) and Timestamp (0x01) bits in FRAME_INFO to attach the frame count and timestamp to the header.
      //
      error = flycaptureSetCameraRegister( g_arContext[ uiCamera ], FRAME_INFO, 0x80000041 );
      _HANDLE_ERROR( error, "flycaptureSetCameraRegister()" );
