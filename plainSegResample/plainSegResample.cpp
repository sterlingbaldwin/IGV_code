...
void viewCloud(cloudPtr);
void viewCloudNormal(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud, pcl::PointCloud<pcl::Normal>::Ptr normals );
cloudPtr plainSeg(cloudPtr);
pcl::PointCloud<pcl::Normal> resample(cloudPtr);
...
main( int /* argc */, char** /* argv */ )
{
    ...
    cloudPtr cloud = save3dPoints( grabbedImage, triclops, disparityImage16, triclopsColorInput ) ;
    // Close the camera and disconnect
    camera.StopCapture();
    camera.Disconnect();
   
    cloud = plainSeg(cloud);
    pcl::PointCloud<pcl::Normal> normalCloud = resample(cloud);
    pcl::io::savePCDFile ("reSampled.pcd", normalCloud);
...
pcl::PointCloud<pcl::Normal> resample(cloudPtr cloud)
{
